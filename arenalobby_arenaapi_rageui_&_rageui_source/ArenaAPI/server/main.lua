...
AddEventHandler("dmcreator:saveMap", function(mapData)
    ...
    if success then
        TriggerClientEvent('chat:addMessage', src, { args = { "ArenaAPI", "Karte erfolgreich gespeichert!" } })
    else
        TriggerClientEvent('chat:addMessage', src, { args = { "ArenaAPI", "Fehler beim Speichern der Karte!" } })
    end
end)
...
AddEventHandler("dmcreator:loadMap", function(mapName)
    local src = source
    local mapData = ArenaAPI.LoadMap(mapName)

    if mapData then
        TriggerClientEvent("dmcreator:loadMapData", src, mapData)
    else
        TriggerClientEvent('chat:addMessage', src, { args = { "ArenaAPI", "Karte konnte nicht geladen werden!" } })
    end
end)
