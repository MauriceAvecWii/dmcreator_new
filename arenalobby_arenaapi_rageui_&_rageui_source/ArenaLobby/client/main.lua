...
-- Deathmatch Creator hinzufügen
SendNUIMessage({
    message = "add",
    item = "dmcreator",
    ownerName = "Server",
    image = "dmcreator", -- Icon für den Deathmatch Creator (muss vorhanden sein)
    label = "Deathmatch Creator",
    state = "",
    players = "1/1", -- Immer verfügbar
    password = "false",
    PlayerAvatar = ""
})

RegisterNUICallback('join', function(data, cb)
    if data.item == "dmcreator" then
        -- Öffnet das Deathmatch Creator-Menü
        TriggerEvent("dmcreator:openMenu")
    else
        -- Bestehende Logik für andere Spiele
        if ArenaAPI:IsPlayerInAnyArena() then
            Citizen.Wait(500)
        end
        ExecuteCommand("minigame join "..data.item)
