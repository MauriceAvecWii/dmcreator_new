...
RegisterServerEvent("dmcreator:saveMap")
AddEventHandler("dmcreator:saveMap", function(mapData)
    local src = source
    local playerId = GetPlayerIdentifier(src)

    -- Karte für den Spieler speichern
    SaveResourceFile("dmcreator", "data/maps/"..playerId..".json", json.encode(mapData), -1)
    TriggerClientEvent('chat:addMessage', src, { args = { "DM Creator", "Karte erfolgreich gespeichert!" } })
end)

RegisterServerEvent("dmcreator:loadMap")
AddEventHandler("dmcreator:loadMap", function(mapName)
    local src = source
    local mapData = LoadResourceFile("dmcreator", "data/maps/"..mapName..".json")
    if mapData then
        TriggerClientEvent("dmcreator:loadMapData", src, json.decode(mapData))
    else
        TriggerClientEvent('chat:addMessage', src, { args = { "DM Creator", "Karte konnte nicht geladen werden!" } })
    end
end)
